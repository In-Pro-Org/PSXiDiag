...
$DBPath = "$RepoPath\pode\db"
$DBFile = Join-Path -Path $DBPath -ChildPath 'KOWESXi.db'
...
function New-SqlLiteDB {
    ...
    param (
        $DBFile,
        $Comment = "This is the PSXi Database"
    )

    if (-not(Test-Path -Path $DBFile)) {
        New-MySQLiteDB $DBFile -Comment $Comment -PassThru -force
    }
}
#endregion
