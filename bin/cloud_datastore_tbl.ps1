...
Add-PodeWebPage -Group $($GroupName) -Name "$($GroupName) Datastore Table" -Title "$($GroupName) Datastore Inventory" -Icon 'cloud' -ArgumentList $GroupName -ScriptBlock {
    param($GroupName)
    ...
    Set-PodeWebBreadcrumb -Items @(
        ...
        New-PodeWebBreadcrumbItem -Name "$GroupName Datastore Inventory" -Url "/pages/PageName?value=$($GroupName) Datastore Table" -Active
    )
